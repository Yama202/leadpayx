-- Consolidação final do fluxo de lote do operador:
-- - rotação por janela de 5 minutos;
-- - lote de 2 contas do mesmo captador (mais antigo primeiro);
-- - exclusividade transacional (FOR UPDATE SKIP LOCKED);
-- - assinatura RPC única para evitar ambiguidade no PostgREST: (uuid,int);
-- - sem gate de "aptidão" de operador.
...
begin
  ...
  -- Para operador: exibe apenas o ciclo liberado para a janela atual.
  ...
  -- Para admin: visão global da fila.
  return query
  select
    a.captador_id,
    coalesce(p.name, p.email, 'Captador')::text as captador_name,
...
as $$
begin
  -- Fallback seguro: não bloqueia o fluxo de atribuição caso
  -- a lógica de redistribuição por SLA não esteja instalada.
  return 0;
end;
$$;
...
begin
  ...
  -- Não deixa ausência/erro dessa função travar a atribuição.
  begin
  ...
  -- Operador alvo precisa existir e estar ativo.
  if not exists (
    select 1
    from public.profiles p
  ...
  -- Janela de rotação de 5 minutos.
  slot_operator_id := public.get_current_operator_rotation_slot();
  ...
  -- Seleciona o captador com ciclo completo mais antigo.
