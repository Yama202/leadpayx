...
import type { PushRegisterActionState } from "@/lib/actions/operator-push";
import {
  registerOperatorPushSubscriptionAction,
  removeOperatorPushSubscriptionAction,
} from "@/lib/actions/operator-push";
import { pushClientsideSupported, urlBase64ToUint8Array } from "@/lib/push-client";

export function OperatorPushSettings(props: {
  ...
  const activate = useCallback(() => {
    ...
    startTransition(async () => {
      try {
        ...
        const result = await registerOperatorPushSubscriptionAction(undefined, fd);
        setState(result);
      } catch (err: unknown) {
  ...
  const deactivate = useCallback(() => {
    startTransition(async () => {
      try {
        ...
        const result = await removeOperatorPushSubscriptionAction(undefined, fd);
        setState(result);
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        setState({ ok: false, message: `Erro: ${msg}` });
  ...
  return (
    <div className="mb-4 flex items-center justify-between gap-4 rounded-2xl border border-white/[0.08] bg-white/[0.04] px-4 py-3">
      <div className="min-w-0">
        <p className="text-sm font-bold text-zinc-200">Notificações push</p>
        <p className="text-xs text-zinc-500">
          {isActive ? "Ativo neste dispositivo — avisa quando chegar conta nova." : "Receba alerta quando um captador enviar conta."}
        </p>
        {state && (
