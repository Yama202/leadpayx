...
export async function registerOperatorPushSubscriptionAction(
  prev: PushRegisterActionState | void,
  fd: FormData,
): Promise<PushRegisterActionState> {
  void prev;
  ...
  const { error } = await supabase.from("operator_web_push_subscriptions").upsert(
    {
      user_id: profile.id,
  ...
  if (error) {
    console.error("[operator-push] upsert falhou", error.message);
    return { ok: false, message: "Não foi possível salvar o registro. Tente de novo." };
  }
...
export async function removeOperatorPushSubscriptionAction(
  prev: PushRegisterActionState | void,
  fd: FormData,
): Promise<PushRegisterActionState> {
  void prev;
  ...
  if (error) {
    console.error("[operator-push] delete falhou", error.message);
