...
export async function AdminDashboardData({ periodParam }: { periodParam: string | undefined }) {
  const periodRange = getPeriodRange(periodParam);
  ...
  const [accounts, payouts, captadores, operadores, financial, ranking, validatedReferrals] =
    await Promise.all([
      ...
      supabase.rpc("get_financial_summary", { period_start: null, period_end: null }).returns<FinancialSummary[]>(),
      supabase.rpc("get_captador_ranking", { period_start: null, period_end: null }).returns<CaptadorRanking[]>(),
      supabase
  ...
  const validatedTotal = validatedRows.reduce((sum, row) => sum + Number(row.total_validated), 0);
          ...
          {validatedRows.length ? (
            validatedRows.map((row, index) => (
              ...
              >
                ...
                <div>
                  <p className="font-black text-slate-950 dark:text-white">{row.name ?? row.email ?? "Captador"}</p>
                ...
                <div className="rounded-2xl bg-emerald-50 p-3 text-center dark:bg-emerald-400/10">
                  <p className="text-xs font-bold text-emerald-700 dark:text-emerald-300">Validados</p>
                  <p className="text-2xl font-black text-emerald-950 dark:text-emerald-100">{row.total_validated}</p>
                </div>
              </div>
            ))
          ) : (
      ...
      <section className="mt-6 rounded-[2rem] border border-white/70 bg-white/85 p-5 shadow-xl shadow-slate-950/5 backdrop-blur dark:border-white/10 dark:bg-slate-900/80">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          ...
          <p className="max-w-xl text-sm leading-6 text-slate-600 dark:text-slate-400">
            Score inteligente: base operacional (volume, conclusão, ganhos, consistência e baixa recusa) + bônus por
          ...
          {intelligentRanking.slice(0, 5).map((row, index) => (
            ...
            >
              ...
              <div>
                <p className="font-black text-slate-950 dark:text-white">{row.name ?? row.email ?? "Captador"}</p>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  {row.completed_accounts}/{row.accounts_submitted} concluídas · {toCurrency(row.generated_amount)}{" "}
                  gerados · {(Number(row.rejection_rate) * 100).toFixed(1)}% recusa · {row.validatedCount} indicações
                  validadas
                </p>
              </div>
              <div className="rounded-2xl bg-emerald-50 p-3 text-center dark:bg-emerald-400/10">
                <p className="text-xs font-bold text-emerald-700 dark:text-emerald-300">Score inteligente</p>
